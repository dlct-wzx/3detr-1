...
class ToSceneDatasetConfig(object):
    def __init__(self):
        self.num_semcls = 53
        self.num_angle_bin = 12
        self.max_num_obj = 53
        self.type2class = {'bag': 0, 'bottle': 1, 'bowl': 2, 'camera': 3, 'can': 4, 'cap': 5, 'clock': 6, 'keyboard': 7, 'display': 8, 'earphone': 9, 
                           'jar': 10, 'knife': 11, 'lamp': 12, 'laptop': 13, 'microphone': 14, 'microwave': 15, 'mug': 16, 'printer': 17, 
                           'remote_control': 18, 'phone': 19, 'alarm': 20, 'book': 21, 'cake': 22, 'calculator': 23, 'candle': 24, 'charger': 25, 
                           'chessboard': 26, 'coffee_machine': 27, 'comb': 28, 'cutting_board': 29, 'dishes': 30, 'doll': 31, 'eraser': 32, 
                           'eye_glasses': 33, 'file_box': 34, 'fork': 35, 'fruit': 36, 'globe': 37, 'hat': 38, 'mirror': 39, 'notebook': 40, 
                           'pencil': 41, 'plant': 42, 'plate': 43, 'radio': 44, 'ruler': 45, 'saucepan': 46, 'spoon': 47, 'tea_pot': 48, 
                           'toaster': 49, 'null': 50,'vase': 51, 'vegetables': 52}
        self.class2type = {self.type2class[t]: t for t in self.type2class}
        self.type2onehotclass = {'bag': 0, 'bottle': 1, 'bowl': 2, 'camera': 3, 'can': 4, 'cap': 5, 'clock': 6, 'keyboard': 7, 'display': 8, 'earphone': 9, 
                           'jar': 10, 'knife': 11, 'lamp': 12, 'laptop': 13, 'microphone': 14, 'microwave': 15, 'mug': 16, 'printer': 17, 
                           'remote_control': 18, 'phone': 19, 'alarm': 20, 'book': 21, 'cake': 22, 'calculator': 23, 'candle': 24, 'charger': 25, 
                           'chessboard': 26, 'coffee_machine': 27, 'comb': 28, 'cutting_board': 29, 'dishes': 30, 'doll': 31, 'eraser': 32, 
                           'eye_glasses': 33, 'file_box': 34, 'fork': 35, 'fruit': 36, 'globe': 37, 'hat': 38, 'mirror': 39, 'notebook': 40, 
...
class ToSceneDetectionDataset(Dataset):
    def __init__(
        self,
        dataset_config,
        split_set="train",
        ...
        if split_set in ["train", "val"]:
            self.scan_names = sorted(
                list(
                    set([os.path.join(self.data_path, x) for x in os.listdir(self.data_path)])
                )
            )
            ...
            for sub_split in sub_splits:
                ...
                basenames = sorted(
                    list(set([os.path.join(self.data_path, x)  for x in os.listdir(data_path)]))
                )
    ...
    def __getitem__(self, idx):
        ...
        npz = np.load(scan_name)
        point_cloud = npz['xyz']                            # Nx3
        pcl_color = npz['color'] / 255.0                    # Nx3
        instance_labels = npz['instance_label']             # Nx1
        semantic_labels = npz['semantic_label']             # Nx1
        instance_bboxes = npz['bbox']                       # K,7
        euler_angles = npz['euler_angle'] * np.pi / 180.0   # K,3
        # convert xyz rgb to xyzrgb format N,6
        point_cloud = np.concatenate([point_cloud, pcl_color], axis=1)  
        # convert xyznxnynzclass z to xyznxnynz z class format K,8
        bboxes = np.concatenate([instance_bboxes, euler_angles[:, 2][:, np.newaxis]], axis=1)
        bboxes[:, [6, 7]] = bboxes[:, [7, 6]]
