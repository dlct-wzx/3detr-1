...
def rotation_matrix_3d(theta_x, theta_y, theta_z):
    """
    生成三维旋转矩阵（绕 x、y、z 轴旋转）
    :param theta_x: 绕 x 轴的旋转角度（弧度）
    :param theta_y: 绕 y 轴的旋转角度（弧度）
    :param theta_z: 绕 z 轴的旋转角度（弧度）
    :return: 3x3 旋转矩阵
    """
    # 绕 x 轴旋转
    Rx = np.array([
...
def generate_rotation_matrices(angles):
    """
    生成 N 个旋转矩阵
    :param angles: 形状为 (N, 3) 的数组，每一行是 (theta_x, theta_y, theta_z)
    :return: 形状为 (N, 3, 3) 的数组，表示 N 个旋转矩阵
    """
    N = angles.shape[0]
    rotation_matrices = np.zeros((N, 3, 3))
    
    for i in range(N):
        theta_x, theta_y, theta_z = angles[i]
        rotation_matrices[i] = rotation_matrix_3d(theta_x, theta_y, theta_z)
...
def merge_linesets(line_sets):
    ...
    merged_lines = np.vstack(all_lines)

    merged_line_set = o3d.geometry.LineSet()
    merged_line_set.points = o3d.utility.Vector3dVector(merged_points)
    merged_line_set.lines = o3d.utility.Vector2iVector(merged_lines)

    return merged_line_set

def write_coordinate_axis(points, rotas, out_filename):
    #绘制坐标轴
    # points[n,3] 为坐标轴的起点
    # rotas[n,3] 为坐标轴的旋转角度
    # filename 为保存的文件名
    # n*3 -> n*4*3
    ans = points.shape[0]
    points = np.repeat(points, 4, axis=0)
    for i in range(len(points)):
        if i % 4 == 1:
            ...
            points[i, 2] += 0.1
    rotm = generate_rotation_matrices(rotas)
    rotm = np.repeat(rotm, 4, axis=0)

    # 点成旋转矩阵
    rotated_points = np.einsum('ijk,ik->ij', rotm, points)
    rotated_points = rotated_points.reshape(-1, 4, 3)
    # 定义线段连接方式 (原点到 X, Y, Z 轴)
    lines = np.array([
        [0, 1],  # 原点到 X 轴
        [0, 2],  # 原点到 Y 轴
        [0, 3]   # 原点到 Z 轴
    ...
    for i in range(len(rotated_points)):
        ...
        line_set.points = o3d.utility.Vector3dVector(rotated_points[i])  # 设置点
        line_set.lines = o3d.utility.Vector2iVector(lines)   # 设置线段
        line_set.colors = o3d.utility.Vector3dVector(colors) # 设置颜色
        line_sets.append(line_set)
    line_sets = merge_linesets(line_sets)


    # 保存为 PLY 文件
    o3d.io.write_line_set(out_filename, line_sets)
